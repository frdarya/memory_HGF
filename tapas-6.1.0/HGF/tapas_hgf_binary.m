...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        % ~~~~~~~~~
        % Prediction
        muhat(k,1) = tapas_sgm(muhat(k,2), 1);% orig: tapas_sgm(ka(1) *muhat(k,2), 1);
        ...
        % Updates
        pi(k,2) = pihat(k,2) +1/pihat(k,1);%orig: pihat(k,2) +ka(1)^2/pihat(k,1);
        mu(k,2) = muhat(k,2) +1/pi(k,2) *da(k,1);%orig: muhat(k,2) +ka(1)/pi(k,2) *da(k,1);
